...
/**
 * @author jingwen.tong 2006-12-18
 *  Copyright IBM 2005
 *  读取配置文件初始化相应的值
 */
public class InitEnvironment {
//	private static final Log logImpl = LogFactory.getLog(InitEnvironment.class);
...
//		    GlobalVar.connectionInfo.setTimeOutTime(Integer.parseInt(mqconinfoElement.getAttributeValue("timeout"))*60*1000);
		    ...
		    if(((Element)mqparamsetList.get(0)).getChild("startpurgethread").getTextTrim().compareToIgnoreCase("1")==0)
		    {
		    	GlobalVar.isStartPurgeThread = true;
		    }
		    else
		    	GlobalVar.isStartPurgeThread = false;
		    GlobalVar.purgeThreadRunInterval = Integer.parseInt(((Element)mqparamsetList.get(0)).getChild("purgethreadruninterval").getTextTrim())*60*1000;
		   ...
		   {
			   throw new Exception("配置文件:"+configFileName+"节点transparamset下的字节点chunktransparamsetList不能存在多个或者不存在！");
		   ...
		   GlobalVar.fileTransControlFinishQueueName = transparamElement.getChild("filetranscontorlfinishqueuename").getTextTrim();
